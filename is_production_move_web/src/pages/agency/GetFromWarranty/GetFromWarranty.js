...
/*
* @description: take product from factory
*/
function GetFromWarranty(props) {
    ...
    React.useEffect(() => {
        UseFetch(`/backend/agency/product/warranty/done`, "GET", null).then((res) => {
    ...
    const onRequestProduct = () => {
        ...
        UseFetch(`/backend/agency/product/warranty/done`, "POST", { "product_id": _id }).then(res => {
            if (res.status.code === "SUCCESS") {
                ...
                alert("Lấy sản phẩm và trả lại thành công")
            } else {
                alert("Lấy sản phẩm và trả lại thất bại")
            }
        })
    }
    ...
    return (
        <React.Fragment>
            <Table title={
                <React.Fragment>
                    <span style={{ marginRight: '1rem' }}>Lấy sản phẩm từ TTBH và trả lại khách hàng</span>
